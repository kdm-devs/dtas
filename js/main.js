...
(function () {
  ...
  // Анимация для страницы 405:
  window.addEventListener('scroll', function(e) {
    const animatingBlocks405 = document.querySelector('.animate-wrap-405');
    if(animatingBlocks405) {
      if (window.pageYOffset >= '2300') {
        animatingBlocks405.classList.add('threeOne-405--hidden');
      } else {
        animatingBlocks405.classList.remove('threeOne-405--hidden');
      }
    }
  });
...
(function () {
  ...
  if (document.querySelector('#threeOne-404')) {
    
    threeOne = document.querySelector('#threeOne-404');
...
// document.getElementById('work-animation-405').appendChild( container );
//   camera = new THREE.PerspectiveCamera( 75, window.innerWidth / window.innerHeight, 1, 10000 );
//   camera.position.z = 1000;
//   scene = new THREE.Scene();
//   particles = new Array();
